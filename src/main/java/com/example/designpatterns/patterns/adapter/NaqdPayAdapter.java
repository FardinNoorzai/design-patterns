...
public class NaqdPayAdapter implements PaymentGateway {
    Logger logger = LoggerFactory.getLogger(NaqdPayAdapter.class);
    @Override
    public PaymentResult charge(String cardNumber, BigDecimal amount) {
        logger.info("Charging Card Number using naqdpay client internal logic: {} amount {}", cardNumber,amount.toString());
    ...
    public RefundResult refund(String cardNumber, BigDecimal amount) {
        logger.info("Refunding Card Number using naqdpay client internal logic: {} amount {}", cardNumber,amount.toString());
